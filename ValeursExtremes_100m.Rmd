...
Nous avons enregistré les meilleurs temps personnels des coureurs les plus rapides entre le 1er janvier 1991 et le 1er avril 2017. 
Pour exclure, autant que possible, les temps liés au dopage, notre période d’observation commence donc début 1991 (mise en place des contrôles anti-dopage en compétition).  
Chaque athlète n’apparaît qu’une fois dans le jeu de données. L’inclusion de plusieurs fois d’un athlète ne serait pas conforme à l’hypothèse de données indépendantes. Seuls les temps oﬃciellement reconnus et les temps avec une vitesse du vent inférieure ou égale à 2m/s sont pris en compte.
Les temps pour le 100m sont disponibles en centièmes de secondes. 

**source** : temps100m.csv qui rassemble :  

  - Pour la période antérieure à 2001, les données proviennant du site Web suédois http://hem.bredband.net/athletics/athleticsall-timebest.htm. Ce site Web fournit tous les meilleurs temps jusqu’en 2001. 
  - Pour la période du 01-01-2001 au 01-04-2017, les listes des temps officiels du site Web de l’IAAF : www.iaaf.org.  

**Objectif :**
Nous aimerions savoir à quelle vitesse pouvons-nous courir? Autrement dit, nous nous intéressons au record mondial ultime.  
...
Le jeu de données ne contient qu'une variable (*temps*) qui recense 1005 relevés des meilleurs temps à la course sur 100m : ils sont entre 9.58 secondes et 10.30 secondes. La moyenne et la médiane de ces temps sont proches et élévées par rapport à la plage de valeurs, respectivement à 10.16s et 10.2s. 
Ici on s'intéresse à la variable *temps* pour ses petites valeurs, donc les valeurs extrèmes seront des minima et la queue de distribution de la série sera pour les temps les plus bas (cf.histogramme).  
...
Speed=360/temps  
La variable *Speed* prend ses valeurs entre 34.93 et 37.58. La médiane et la moyenne, respectivement de 35.31 et 35.45, sont proches de la valeurs minimum, ce qui écrasse la série (concentration des performances vers les valeurs basses). Les performances de 8 athlètes se dégagent des autres, dépassant les 36.7km/h au 100m.
...
La valeur estimée de gamma (shape) est de 0.363 > 0 (avec un intervalle de confiance aux bornes positives). Elle caractérise le domaine d'attraction de Fréchet.
Les 4 graphiques montrent un ajustement non convainquant de la loi des extrêmes au DMA de Fréchet : les points empiriques et les courbes représentant le modèle sont éloignés pour des vitesses supérieurs à 36km/h.
...
Une autre méthode pour évaluer et selectionner le seuil est de tester plusieurs valeurs en fonction du nombre d'excès observés au delà de celui-ci (fonction *findThreshold*) . 
...
```
  ...
  - 1 coureur (0.1%) a atteint : 37.16 km/h (correspond au max de la série)  
  - 8 coureurs (1%) ont atteint : 36.66 km/h
  - 20 coureurs (2%) ont atteint : 36.50 km/h  
  - 100 coureurs (10%) ont atteint : 36.06 km/h  
  
Sélectionner un seuil qui définit autour de 1%-2% de valeurs extremes peut être un choix raisonnable.  On peut retenir 36.50 km/h ou 36.66 km/h.  
...
Il apparait sur ces 2 graphiques, une rupture de stabilité des paramètres pour un seuil autour de 36.5km/h. Choisissons cette valeur pour seuil.
...
La valeur estimée de gamma (shape) est de 0.47> 0 . Elle caractérise la domaine d'attraction de Fréchet. Cependant l'intervalle de confiance du paramètre gamma nous indique qu'il pourrait être négatif (borne inférieure du CI <0). Nous testerons ci-après, le résultat de la modélisation pour gamma = 0 (domaine de Gumbel).  
...
Les graphiques d'ajustement donnent de moins bons résultats qu'avec les modélisations précédentes. Nous ne retiendrons pas Gumbel.
...
La valeur estimée de gamma (shape) est positive (0.408> 0). Fréchet ressort encore comme le domaine d'appartenance de la distribution des extrêma.  
