...
# Sujet : niveaux maximum de la mer dans la région de Port Pirie, au Australie du Sud
...
Il s'agit d'obtenir une estimation du niveau maximal de la mer susceptible de se produire dans la région au cours des 100 ou 1000 prochaines années. On supposera que le modèle est constant sur la période d’observations, de sorte que nous modélisons les données comme des observations indépendantes de la distribution du GEV : distribution des extrêmes généralisés.


# Description de la série

La série comprend 65 de niveaux maximum annuels de la mer (mètre), relevés entre 1923 et 1987.
Les niveaux sont compris entre 3.57m et 4.69m, avec une moyenne et une médiane proches (resp.3.98m et 3.96m), ce qui est confirmé par le boxplot reparti autour de la moyenne. Seules 3 valeurs sortent franchement du nuage de points : 4.69m en 1934, 4.55m en 1953 et 1981. 
...
La valeur estimée de gamma (shape) est de -0.0501 < 0. Elle caractérise le domaine d'attraction de Weibull. Cependant l'intervalle de confiance du paramètre gamma nous indique qu'il pourrait être positif (borne supérieure du CI >0). Nous testerons donc ci-après, le résultat de la modélisation pour gamma = 0 (domaine de Gumbel).   
Les 4 graphiques montrent un bon ajustement de la loi des extrêmes au DMA de Weibull (points empiriques et modélisés proches, relation niveaux et période de retour proches des valeurs de la loi théorique).


**Point terminal**  
Le point terminal est la limite supérieur atteinte par le domaine d'attraction. Il s'obtient à partir des paramètres estimés: position - (échelle/forme)  
...
Le point terminal est estimé ici à : `r PPGEVMle.endp`

## Modèle Gumbel avec MLE 

Nous testons ici l'appartenance du modèle des extrêmes au domaine de Gumbel, c'est à dire  le paramètre de forme (gamma) peut-il être nul?
...
Les graphiques d'ajustement de la loi de niveau maximum vers une loi du domaine de Gumbel  donnent de très bons résultats. L'hypothèse que le paramètre de forme, gamma, soit nul est donc possible. 
De plus, les intervalles de confiance des niveaux de retour à 100 ans et 1000 ans sont très resserrés autour de la valeur ponctuelle estimée, ce qui marque une précision forte dans cette estimation.
...
La valeur estimée de gamma (shape) est là aussi négative (=-0.05).
Les 4 graphiques montrent là aussi un bon ajustement de la loi des extrêmes au DMA de Weibull.

**Point terminal**

Le point terminal est dans ce cas de : `r PPGEVMom.endp`


# Conclusion

```{r tab}
table_ccl=data.frame(methode_d_estimation=c("GEV-MLE","Gumble-MLE","GEV-Moment"), estimation_param_forme=c("-0.05","0","-0.05"), domaine=c("Weibull","Gumbel","Weibull"),point_final=c("7.60m","X","7.82m"),niv_retour_100_ans=c("X","4.77m ","X"),niv_retour_1000_ans=c("X","5.22m","X"))
...
Il semble que ceux obtenus par le Gumbel et l'estimation des paramètres par le maximum de vraissemblance soient légèrement meilleurs . On retiendra donc ce modèle.


**Modèle Gumbel avec estimation par maximum de vraissenblance**  
   
  **- Domaine = Gumbel** 
  **- Estimation position = 3.87**   
  **- Estimation echelle = 0.19**  
  **- Estimation forme = 0**  
  **- Niveau de retour à 100 ans = 4.77**   
  **- Niveau de retour à 1000 ans = 5.22**   
